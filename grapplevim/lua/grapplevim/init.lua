...
-- Date: June 25, 2025 (Hybrid Input Model)
...
local State = {
	...
	-- NEW: Input state is back, but ONLY for movement keys
	input = {
		j = false,
		k = false,
		h = false,
...
-- Config for the hybrid model
local Config = {
	step_rate = 33,
	key_release_timeout = 60, -- For hjkl hold detection
	gravity = 0.02,
	grapple_tension = 0.03,
	grapple_friction = 0.98,
	-- These are continuous forces applied while hjkl are held.
	j_force = 0.08,
	k_force = 0.08,
	hl_force = 0.08,
	-- Visuals
...
local function find_nearest_anchor()
	...
	local start_line, start_col = start_pos[1], start_pos[2] + 1
	local max_lines = vim.api.nvim_buf_line_count(0)
	local best_pos = nil
	local min_dist_sq = math.huge
	for i = 0, max_lines do
		for _, sign in ipairs({ 1, -1 }) do
			local current_line_idx = start_line + (i * sign)
			if current_line_idx >= 1 and current_line_idx <= max_lines then
				local line_content = vim.api.nvim_buf_get_lines(0, current_line_idx - 1, current_line_idx, false)[1]
					or ""
				if line_content:match("%S") then
					local closest_col_on_line = nil
					for d = 0, line_content:len() do
						local col_right, col_left = start_col + d, start_col - d
						if col_right <= line_content:len() then
							if (line_content:sub(col_right, col_right)):match("%S") then
								closest_col_on_line = col_right
								break
							end
						end
						if col_left >= 1 then
							if (line_content:sub(col_left, col_left)):match("%S") then
								closest_col_on_line = col_left
								break
							end
						end
					end
					if closest_col_on_line then
						local dist_sq = (current_line_idx - start_line) ^ 2 + (closest_col_on_line - start_col) ^ 2
						if dist_sq > 0 and dist_sq < min_dist_sq then
							min_dist_sq = dist_sq
							best_pos = { y = current_line_idx, x = closest_col_on_line }
						end
					end
				end
			end
		end
		if best_pos and i > math.sqrt(min_dist_sq) then
			return best_pos
		end
	end
	return best_pos
...
-- NEW: process_input is back for smooth movement
...
-- The update loop now processes movement input again
...
-- ====================================================================
-- 3. State Management and Keymaps (Hybrid Model)
-- ====================================================================

-- The key-hold simulation functions are back
...
-- Grapple remains a simple toggle function
...
local function setup_gravity_maps()
	...
	-- Grapple key uses the simple toggle function
...
-- We need to stop the input timers again on exit
